...
embedded_ds = ds.map_batches(
    EmbeddingWorker,
    compute=ray.data.ActorPoolStrategy(min=2, max=4),
    batch_size=32
)
```
...
2. **Normalize Vector Embeddings**: Normalize embeddings ($L_2$ norm = 1.0) so cosine similarity can be computed via high-speed dot products.
...
| :--- | :--- | :--- |
| `data_genai01` | Build a distributed text chunking and preprocessing pipeline | [**Open Exercise data_genai01 →**](../playground/index.html?exercise=data_genai01) |
| `data_genai02` | Deploy an embedding model actor pool with `ActorPoolStrategy` | [**Open Exercise data_genai02 →**](../playground/index.html?exercise=data_genai02) |
| `data_genai03` | Construct a distributed vector similarity search engine | [**Open Exercise data_genai03 →**](../playground/index.html?exercise=data_genai03) |
| `data_genai04` | Implement end-to-end multimodal retrieval and context ranking | [**Open Exercise data_genai04 →**](../playground/index.html?exercise=data_genai04) |
